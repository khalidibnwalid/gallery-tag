...
export function setupGlobalNotifier(queryClient?: QueryClient) {
  if (!window.api || !window.api.general.onNotify)
    return // Unsubscribe from any previous window-level listener (prevents duplicates during HMR)
  ;(window as any).__globalNotifyUnsubscribe?.()

  const unsubscribe = window.api.general.onNotify(notification => {
  ...
  ;(window as any).__globalNotifyUnsubscribe = unsubscribe

  return () => {
    if ((window as any).__globalNotifyUnsubscribe === unsubscribe) {
      unsubscribe()
      ;(window as any).__globalNotifyUnsubscribe = null
    }
  }
}
