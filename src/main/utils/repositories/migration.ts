...
const migrationModules = import.meta.glob('./migrations/*.ts', {
  eager: true,
}) as Record<string, { up: (db: Database.Database) => void }>
...
export function runMigrations(db: Database.Database): void {
  ...
  console.log(
    `Database version: ${currentVersion}. Target version: ${targetVersion}.`,
  )
  ...
  const applyMigrations = db.transaction(
    (fromVersion: number, toVersion: number) => {
      for (let i = fromVersion; i < toVersion; i++) {
        console.log(
          `Applying migration v${i + 1} (${sortedMigrationKeys[i]})...`,
        )
        migrations[i](db)
